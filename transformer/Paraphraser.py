...
class Inference(object):
    ...
    def paraphrase_batch(self, src_seq, src_pos):
        ...
        def collate_active_info(src_seq, src_enc, inst_idx_to_tensor_position_map, active_inst_idx_list):
            n_prev_active_inst = len(inst_idx_to_tensor_position_map)
        ...
        def funny(inst_dec_beams):
            print("fun",inst_dec_beams)
            return 0
        ...
        with torch.no_grad():
            ...
            z = torch.randn_like(src_enc[:,:,:64])
            src_enc = torch.cat((z, src_enc), 2)
